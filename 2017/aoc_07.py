...
# Parses a line into a node in the graph.
def parse_line(l):
    ps = l.split()
    name = ps[0]
    weight = ps[1][1:-1]
...
# By definition, the root node is the node that has no parents.
# Our nodes are of the shape (name, weight, parents)
nodes = [parse_line(r) for r in rs]

# Also, nodes without parents are leaves and therefore cannot be the root.
root_candidates = filter(lambda x: len(x[2]) > 0, nodes)
all_parents = {}
for node in root_candidates:
    _0, _1, ps = node
    for p in ps:
        # Strip commas from our shitty lexing/parsing.
        if ',' in p:
            p = p[:-1]
        all_parents[p] = True

for node in root_candidates:
    if node[0] not in all_parents:
        print(node[0])
        break
